...
class BaseTrainer(object):
    def __init__(self, model, trackbook,ema_model, criterion, summary_writer, prob=0.5, mean=[0.4914, 0.4822, 0.4465]):
    ...
    def train(self, epoch, data_loader, optimizer, random_erasing, alpha=0.0, empty_cache=False, print_freq=5):
        ...
        for i, inputs in enumerate(pbar):
            ...
            loss_student,  all_loss, prec_student, domain_acc_student, prec_teacher = self._forward(
                [imgs_student],[imgs_teacher], targets, domain_ids, alpha,cons_weight=cons_weight,clip_weight=clip_weight)
            #
            ...
            loss_clip_m.update(all_loss[6].item(), targets.size(0))  
...
class ReidTrainer(BaseTrainer):
    ...
    def _forward(self, inputs_s, inputs_t, targets, domain_ids, alpha, cons_weight,clip_weight):
        
        outputs_student = self.model(inputs_s, alpha=alpha, training=True)
        img_embed = outputs_student[4]
        ...
        loss_clip = clip_weight * self.criterion[4](img_embed, text_embed, targets)
        ...
        loss_student = loss_cls_student + loss_tri_student + loss_domain_student * 2.5  + loss_kl + loss_clip
        losses = [loss_cls_student, loss_tri_student, loss_domain_student, loss_cls_teacher, loss_tri_teacher, loss_kl,loss_clip]
