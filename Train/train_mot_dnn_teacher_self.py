...
from reid.loss.loss_set import CrossEntropyLabelSmoothLoss, TripletHardLoss,RelationKLLoss,CenterCosineLoss
from torch.optim.lr_scheduler import CosineAnnealingLR
from Train.Model.EMAModel import EMAModel
from Train.engine.reid_trainers_dnn_teacher_self import ReidTrainer  
...
def parse_args():
    ...
    parser.add_argument('-b', '--batch_size', type=int, default=72)
    ...
    parser.add_argument('--start_show', type=int, default=20)  
    ...
    parser.add_argument("--output_dir", type=str, default='/home/st3/UnifyTrack/appearance/vis2/u3')
    
    parser.add_argument('--dist-metric', type=str, default='cosine', choices=['euclidean', 'kissme'])  
...
def main(args):
    ...
    raw_weights = 1.0 / (np.array(num_ids_per_domain) + 1e-6)  
    ...
    criterion = [criterion_cls, criterion_tri, criterion_domain, criterion_kl]
    ## Optimizer
    # backbone_params = []
    # reid_head_params = []
    # domain_params = []
    ...
    trainer = ReidTrainer(model=model, ema_model=ema_model, criterion=criterion,summary_writer=summary_writer)  # 将EMA模型传入训练器
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if (epoch + 1) % 10 == 0 and (epoch + 1) >= args.start_save:
            ...
            }, epoch + 1, is_best, save_interval=1, fpath=osp.join(args.logs_dir, 'u3S.pth.tar'))
            ...
            save_checkpoint({
                ...
                'epoch': epoch + 1,
                'best_top1': best_top1,
            }, epoch + 1, is_best, save_interval=1, fpath=osp.join(args.logs_dir, 'u3T.pth.tar'))
